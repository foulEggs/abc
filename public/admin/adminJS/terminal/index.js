(function (window) {	
	...
	function get_list (currentpage, currentline, follow) {
		...
		var form_search = $('#form-search').serialize();

		var search_arr = form_search.split('&');

		var search = [], searchFields = [];
		...
		if (submit_flag === false) {
			...
			$.ajax({
				url     :main_addr+'?page='+currentpage+'&limit='+currentline+'&search='+search.join(';')+'&searchFields='+searchFields.join(';'),
				type    :'get',
				//data    :search,
				dataType:'json',
			...
			$.ajax({
				...
				success:function (result) {
					...
					if (result['deleted'] == true) {
						...
						toastr.success(result.msg, "", load_opts);
