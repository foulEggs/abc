(function (window) {
	...
	function get_list (currentpage, currentline, follow) {
		...
		if (submit_flag === false) {
			...
			$.ajax({
				url     :main_addr
				+'operation_index?page='+currentpage
				+'&limit='+currentline
				+'&search='+search.join(';')
				...
				success:function (result) {
					...
					if (count > 0) {
						for (var i = 0; i < count; i++) {
							//按钮处理
							if(datas[i]['trade_status'] == 3){
								
								datas[i]['operation_btn'] = '<button  type="button" class="btn btn-danger btn-xs"><i class="fa fa-folder"></i>补缴</button>';
							}else if(datas[i]['trade_status'] == 1 || datas[i]['trade_status'] == 2){
								//datas[i]['operation_btn'] = '<button  type="button" class="btn btn-primary btn-xs"><i class="fa fa-folder"></i>补缴</button>';
								datas[i]['operation_btn'] = "";
							}else if(datas[i]['trade_status'] == 5){
								datas[i]['operation_btn'] += '<button  type="button" class="btn btn-info btn-xs"><i class="fa fa-pencil"></i>冲正</button>';
							}
