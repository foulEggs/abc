(function (window) {	
	...
	$("#form-search").find('select[name="clear_type"]').change(function(){
		var charge_type = $(this).val();
		
		charge_type == "district" ? $(".district").show() : $(".district").hide();

		charge_type == "salesman" ? $('.salesman').show() : $('.salesman').hide();
	});

	$("#form-search").find('select[name="clear_type"]').trigger('change');
	...
	function get_list (currentpage, currentline) {
