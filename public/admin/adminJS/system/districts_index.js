(function (window) {	
	...
	var main_addr = '/system/districts';
	...
	function get_list (currentpage, currentline, follow) {
		...
		//console.log($('button.btn-back').attr('level'));
		...
		if (submit_flag === false) {
			...
			var parent_id = sessionStorage.getItem('current_id');
			parent_id = parent_id?parent_id:0;

			$.ajax({
				url     :main_addr+'?page='+currentpage+'&limit='+currentline+'&searchFields=parent_id:=&search=parent_id:'+parent_id,
				type    :'get',
				data    :search,
				dataType:'json',
				success:function (result) {
					...
					if (count > 0) {
						...
						$('button.btn-child').on('click',function(){
							//$('input[name="parent_id"]').val($(this).attr('dataid'));
							$('button.btn-back').show();
							var current_level = sessionStorage.getItem('current_level') ? sessionStorage.getItem('current_level') : 1;
							sessionStorage.setItem('current_level',parseInt(current_level)+1);
							sessionStorage.setItem('current_id',$(this).attr('dataid'));
							sessionStorage.setItem('current_parent_'+$(this).attr('dataid'),$(this).attr('parent_id'));
	...
	$('button.btn-back').on('click',function(){
		...
		var current_level = sessionStorage.getItem('current_level');
		sessionStorage.setItem('current_level',parseInt(current_level)-1);

		var bid = sessionStorage.getItem('current_parent_'+sessionStorage.getItem('current_id'));
		
		sessionStorage.setItem('current_id', bid);
		get_list(1,Paging.GetPageLines());
	...
	function save_data () {
		...
		if (submit_flag === false) {
			...
			if (form_validate.form()) {
				...
				//var _url = data_form.find('input[name="id"]').val()>0?'ajax_edit':'ajax_add';
				var _url = id>0?'/'+id:'';
				$.ajax({
					url :main_addr+_url,
					type: (id>0?'put':'post'),
					...
					success:function (result) {
						...
						if (result['error'] == undefined) {
							...
							get_list(page_objs['index'], page_objs['line'], (id==0?true:false));
	...
	function pop_modal (pop_id, backdrop,action) {
		...
		modal_pop.find('input[type="hidden"][name="parent_id"]').val(sessionStorage.getItem('current_id')?sessionStorage.getItem('current_id'):0);
		modal_pop.find('input[type="hidden"][name="level"]').val(sessionStorage.getItem('current_level')?sessionStorage.getItem('current_level'):1);
	...
	sessionStorage.getItem('current_level')>1?$('button.btn-back').show():$('button.btn-back').hide();
