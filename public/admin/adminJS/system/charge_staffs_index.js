(function (window) {	
	...
	var main_addr = '/system/charge_staffs';
	...
	//$('select[name="terminal_id"]').searchableSelect();
					...
					for(var k in data){
						if(k !== "pwd"){
							...
							if(k == "city" || k =="district"){
								modal_pop.find('select[name="'+k+'"]').trigger('change');
							}
							if(k=="qrcode"){
								console.log(modal_pop.find('.qrcode').find('img'))
								modal_pop.find('.qrcode').find('img').attr('src',data[k]);
							}
							if(k=="terminals"){
								modal_pop.find('select[name="terminal_id"]').val(data[k][0]['id']);
							}
						}
					}
	...
	function pop_modal (pop_id, backdrop,action) {
		...
		if(action == "view"){
			...
			modal_pop.find('.qrcode').show();
		}else{
			var title = "编辑";
			modal_pop.find('*[name]').attr('disabled',false);
			modal_pop.find('.btn-save').show();
			if(action == 'add'){
				modal_pop.find('input[name="pwd"]').attr('placeholder','不填为默认');
				modal_pop.find('.qrcode').hide();
				$('select[name="terminal_id"]').searchableSelect();
			}else{
				modal_pop.find('input[name="pwd"]').attr('placeholder','不填则不修改');
				modal_pop.find('.qrcode').show();
			}
		}
		...
		modal_pop.find('select[name="city"]').trigger('change');
	...
	$('button.btn-add').on('click', function () {
		pop_modal.call(this, 'edit-detail','','add');
	});
